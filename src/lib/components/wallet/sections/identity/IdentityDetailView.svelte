...
<div class="mx-auto flex w-full max-w-4xl min-w-0 flex-col gap-5 px-6 pt-3 pb-6">
  <div class="flex items-center justify-between gap-3">
    ...
    <Button variant="destructive" size="sm" onclick={onUnlink} disabled={unlinking} class="h-8">
  ...
  <section class="space-y-2 rounded-xl bg-muted/20 p-4">
    <h3 class="text-sm font-semibold text-foreground">
      {i18n.t('wallet.identity.detail.sections.base')}
    </h3>
    ...
    <div class="space-y-2 text-sm">
      ...
      >
        <div class="min-w-0">
          <p class="text-xs text-muted-foreground">
            {i18n.t('wallet.identity.detail.fields.name')}
          </p>
      ...
      >
        <div class="min-w-0">
          ...
          {#if details.system}
            <IdentifierText value={details.system} mode="full" class="block text-foreground" />
          {:else}
            <p class="text-foreground">{showValue(details.system)}</p>
          {/if}
        </div>
      </div>
    </div>
  ...
  <section class="space-y-2 rounded-xl bg-muted/20 p-4">
    <h3 class="text-sm font-semibold text-foreground">
      {i18n.t('wallet.identity.detail.sections.authorities')}
    </h3>
      ...
      >
        <div class="min-w-0">
          ...
          {#if details.revocationAuthority}
            <IdentifierText
              value={details.revocationAuthority}
              mode="full"
              class="block text-foreground"
            />
          {:else}
            <p class="text-foreground">{showValue(details.revocationAuthority)}</p>
          {/if}
        </div>
        {#if details.revocationAuthority}
          <CopyButton
      ...
      >
        <div class="min-w-0">
          ...
          {#if details.recoveryAuthority}
            <IdentifierText
              value={details.recoveryAuthority}
              mode="full"
              class="block text-foreground"
            />
          {:else}
            <p class="text-foreground">{showValue(details.recoveryAuthority)}</p>
          {/if}
        </div>
        {#if details.recoveryAuthority}
          <CopyButton
  ...
  <section class="space-y-2 rounded-xl bg-muted/20 p-4">
    ...
    {#if details.primaryAddresses.length === 0}
      <p
        class="rounded-md bg-background/55 px-3 py-2 text-sm text-muted-foreground dark:bg-background/40"
      >
        ...
        {#each details.primaryAddresses as address, index (address)}
          ...
          >
            <IdentifierText
              value={address}
              mode="full"
              class="block min-w-0 flex-1 text-sm text-foreground"
            />
            <CopyButton
  ...
  {#if details.privateAddress}
    <section class="space-y-2 rounded-xl bg-muted/20 p-4">
      <h3 class="text-sm font-semibold text-foreground">
        {i18n.t('wallet.identity.detail.sections.privateAddress')}
      </h3>
      <div
        class="flex items-center justify-between gap-3 rounded-md bg-background/55 px-3 py-2 dark:bg-background/40"
      >
        <IdentifierText
          value={details.privateAddress}
          mode="full"
          class="block min-w-0 flex-1 text-sm text-foreground"
        />
        <CopyButton
  ...
  <section class="space-y-2 rounded-xl bg-muted/20 p-4">
    <h3 class="text-sm font-semibold text-foreground">
      {i18n.t('wallet.identity.detail.sections.warnings')}
    </h3>

    <div class="grid gap-2 md:grid-cols-3">
      <div
        class={`rounded-md px-3 py-2 text-xs ${spendAndSignWarning ? 'bg-amber-500/10 text-amber-700 dark:text-amber-200' : 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-200'}`}
      >
      ...
      <div
        class={`rounded-md px-3 py-2 text-xs ${revokeAuthorityExternal ? 'bg-amber-500/10 text-amber-700 dark:text-amber-200' : 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-200'}`}
      >
      ...
      <div
        class={`rounded-md px-3 py-2 text-xs ${recoveryAuthorityExternal ? 'bg-amber-500/10 text-amber-700 dark:text-amber-200' : 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-200'}`}
      >
    ...
    {#if details.warnings.length > 0}
      <div
        class="space-y-1 rounded-md bg-amber-500/10 px-3 py-2 text-xs text-amber-700 dark:text-amber-200"
      >
    ...
    {:else}
      <p
        class="flex items-center gap-2 rounded-md bg-emerald-500/10 px-3 py-2 text-xs text-emerald-700 dark:text-emerald-200"
      >
