...
impl StrongholdStore {
    ...
    /// last-used writes. The owned gate stays inside the worker if its caller is cancelled.
    /// Only this record family uses this path; encryption parameters remain unchanged.
    pub(crate) async fn update_address_book<T: Send + 'static>(
        &self,
        account_id: &str,
        password_hash: &[u8],
        guard: Option<crate::core::auth::session::SessionSubmissionGuard>,
        update: impl FnOnce(&mut crate::types::AddressBookSnapshot) -> Result<T, WalletError>
            + Send
            + 'static,
    ) -> Result<T, WalletError> {
        static GATE: std::sync::LazyLock<std::sync::Arc<tokio::sync::Mutex<()>>> =
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let original = client
                .store()
                .get(ADDRESS_BOOK_RECORD_KEY)
                .map_err(|_| WalletError::OperationFailed)?
                .map(Zeroizing::new);
            let mut snapshot = match original.as_ref() {
                Some(raw) => serde_json::from_slice::<crate::types::AddressBookSnapshot>(raw)
                    .map_err(|_| WalletError::OperationFailed)?,
                None if !exists => crate::core::address_book::manager::empty_snapshot(),
                None => return Err(WalletError::OperationFailed),
            };
            if !(1..=2).contains(&snapshot.schema_version) {
                return Err(WalletError::OperationFailed);
            }
            ...
            if original.as_ref() == Some(&payload) || (!exists && snapshot.contacts.is_empty()) {
                return Ok(result);
            }
            client
                .store()
                .insert(ADDRESS_BOOK_RECORD_KEY.to_vec(), payload.to_vec(), None)
                .map_err(|_| WalletError::OperationFailed)?;
...
mod tests {
    ...
    #[tokio::test]
    async fn contacts_atomic_updates_preserve_concurrent_records_and_fail_closed() {
        let _ = iota_stronghold::engine::snapshot::try_set_encrypt_work_factor(0);
        let store = temp_store();
        let password = StrongholdStore::derive_legacy_password_hash("synthetic-contact-test");
        let account = "contacts-test";
        let mut jobs = Vec::new();
        for index in 0..4 {
            let store = store.clone();
            let password = password.clone();
            jobs.push(tokio::spawn(async move {
                store.update_address_book(account, &password, None, move |snapshot| {
                    let contact: crate::types::AddressBookContact = serde_json::from_value(serde_json::json!({ "id": format!("contact{}", index), "displayName": "Private test label", "note": "Private test note", "createdAt": 1, "updatedAt": 1, "endpoints": [] })).unwrap();
                    snapshot.contacts.push(contact); Ok(())
                }).await
            }));
        }
        for job in jobs {
        ...
        let count = store
            .update_address_book(account, &password, None, |snapshot| {
                Ok(snapshot.contacts.len())
            })
            .await
            .unwrap();
        assert_eq!(count, 4);
        let before = std::fs::read(store.address_book_snapshot_path(account)).unwrap();
        assert!(!before
            .windows(b"Private test label".len())
            .any(|window| window == b"Private test label"));
        let failed = store
            .update_address_book(account, &password, None, |snapshot| {
                snapshot.contacts.clear();
                Err::<(), _>(WalletError::AddressBookInvalidInput)
            })
            .await;
        assert!(failed.is_err());
        assert_eq!(
            std::fs::read(store.address_book_snapshot_path(account)).unwrap(),
            before
        );
        std::fs::write(store.address_book_snapshot_path(account), b"corrupt").unwrap();
        assert!(store
            .update_address_book(account, &password, None, |_| Ok(()))
            .await
            .is_err());
        assert_eq!(
            std::fs::read(store.address_book_snapshot_path(account)).unwrap(),
